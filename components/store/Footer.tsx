...
export default function Footer() {
    ...
    const values = [
        { icon: Truck, title: "Global Shipping", desc: "Premium delivery to your doorstep" },
        { icon: CreditCard, title: "Secure Payment", desc: "100% encrypted transactions" },
        { icon: Lock, title: "Data Privacy", desc: "Your information is safe with us" },
    ];
    ...
    return (
        <footer className="bg-primary text-white pt-24 pb-8">
            <div className="container mx-auto px-6">
                {/* Value Bar */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-12 pb-20 mb-20 border-b border-muted/10">
                    {values.map((v, i) => (
                        <div key={i} className="flex flex-col items-center text-center space-y-4">
                            <div className="p-4 bg-white/5 rounded-full">
                                <v.icon size={28} className="text-accent" />
                            </div>
                            <h4 className="text-sm uppercase tracking-widest font-bold">{v.title}</h4>
                            <p className="text-xs text-muted/80 tracking-wide">{v.desc}</p>
                        </div>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
                    ...
                    <div className="space-y-6">
                        <h2 className="premium-serif text-3xl font-bold tracking-tighter italic text-gradient">
                            LUMERA
                        </h2>
